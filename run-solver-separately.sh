...
# - Results go to {config_dir}/{workload-only}/ subdirs
# - Example: config/large/prefill-only/, config/large/decode-only/

# config_dir_list=("config/medium" "config/large")
config_dir_list=("config/large")

# Paired config + workload phase
# Format: "config_dir:workload_phase"
# This ensures prefill uses the base config and writes to prefill-only/
# and decode uses the base config and writes to decode-only/
config_workload_pairs=(
    "config/large:prefill"
    "config/large:decode"
)
...
solver="solver_separately.py"

# Iterate over config-workload pairs
for pair in "${config_workload_pairs[@]}"; do
    config_dir=$(echo ${pair} | cut -d':' -f1)
    workload_phase=$(echo ${pair} | cut -d':' -f2)
        ...
        for network_config in "${network_config_list[@]}"; do
            for io_pair in "${io_length_pairs[@]}"; do
                ...
                for batch_pair in "${batch_size_pairs[@]}"; do
                    ...
                    # New output directory structure: config/{size}/{workload}-only/
                    workload_subdir="${workload_phase}-only"
